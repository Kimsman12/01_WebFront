...
document.querySelector("#btn1").addEventListener("click", ()=>{
  ...
  // 2. 첫번째 자식 노드찾기 (요소.firstChild)
  // -> #test(ul)의 첫번째 자식노드 찾기
  ...
  // (요소.childNodes[index])
  // -> #test의 자식 노드중 3번째 lt태그를 찾아서 배경색 pink로 지정하기
  ...
  // -> 탐색용 코드는 연달아서 작성 가능
  // test.childNodex[9] 의 다음 다음 형제노드 선택
  // == test.childNodes[11]
  ...
  // 3. 첫번째 자식으로 추가 (prepend( "내용"or 노드or 요소*n ))
  list[11].prepend("오늘", "점심", "뭐먹지?");

  // 4. 이전/다음 형제로 추가 (before/after("내용"or 노드or 요소*n ))
  // #test 전/후에 내용추가
  test.before("이전 입니다");
  test.after("이후 입니다");
});
