...
document.querySelector("#btn2a").addEventListener("click",function(){
  ...
  // -> 함수 수행 후 돌아와서도 참조하고 있던 배열의 2번인덱스 값이 변화되어 있다

  // 참조란
  /* 
    - 객체 지향 언어에서 배열, 객체 등 값을 여러개 가지고 있는 자료형인데
    변수는 값을 1개만 저장할 수 있기 때문에 object를 메모리 다른곳에 저장하고
    저장된 곳의 주소를 변수에 저장해서 필요할때마다 해당 주소를 찾아가
...
document.querySelector("#btn3a").addEventListener("click", function(){
  ...
  const numbers = []; // 빈 배열생성
  
  // 전달 받은 2개의 값을 곱해서 리턴(반환)
  function returnTest(num1, num2){
    return num1*num2;  // 함수 종료+return 옆에 작성된 값을 가지고 호출한쪽으로 돌아감
  }
  ...
  numbers.push(50);
  numbers.push(4);
  numbers.push(returnTest(2, 5)); // 함수를 호출해서 되돌려받은 값(return 값) 을 push
...
function printObject(otherFn){
  ...
  /* otherFn :
      (name, age) => {return {"name":name, "age":age}} */
