...
output.innerText = balance; // 화면에 초기금액 설정
...
function deposit(){
  ...
  //(번외) : input 에 금액을 작성 안하고 입금버튼 누른경우
  if(amount.value.length == 0) { // 금액을 입력 안했을때
    ...
    // input에 기존 작성된 값 제거하기
    amount.value= ' ';

  }


  // -> 금액을 입력하세요 와 같은 메세지 띄우기
}

//출금
function withdrawal(){
  // 1. 출금 버튼 클릭 해당 함수 수행
  //(번외) : input 에 금액을 작성 안하고 출금버튼 누른경우
  // -> 금액을 입력하세요 와 같은 메세지 띄우기
  // 2. 비밀번호를 입력받기
  // 2-1. 취소버튼 눌렀을 때 처리
  // 2-2. 확인버튼 눌렀을 떄 처리
  ...
  // -> 출금할 금액이 잔액보다 적거나 같은경우 (출금 처리 가능)
  ...
  if(amount.value.length == 0){ // 금액 입력 안했을때 
    ...
    if(pw == null) { // 취소버튼 누른경우
      alert("출금을 취소하셨습니다");
    }else { // 확인버튼 누른경우
      
      
      if(password != pw){ // 비밀번호가 일치 하지 않을경우
